...
style_dict = {
        'style' : {
            'obs' : { 'LineWidth' : 2},
            'exp0' : { 'LineColor' : ROOT.kBlack, 'LineStyle' : 2},
            'exp1' : { 'FillColor' : ROOT.TColor.GetColor("#607641"), 'FillColorAlpha' : [ROOT.TColor.GetColor("#607641"),1.0]},
            'exp2' : { 'FillColor' : ROOT.TColor.GetColor("#F5BB54"), 'FillColorAlpha' : [ROOT.TColor.GetColor("#F5BB54"),1.0]},
            },
        'legend' : {
            'obs' : { 'Label' : 'Observed', 'LegendStyle' : 'LP', 'DrawStyle' : 'PLSAME'},
...
# Style and pads
ModTDRStyle()
canv = ROOT.TCanvas("limit","limit",800,900)
canv.SetTicks(1)
pads = OnePad()

# Get limit TGraphs as a dictionary

masses = sorted([int(f.split("A_")[1]) for f in os.listdir(options.folder+"/all/")])
...
for ind, mass in enumerate(masses):
  ...
  graphs.append(StandardLimitsFromJSONFile_wScaling(options.folder+'/all/A_'+ str(mass) + '/cmb/limits/limit_limits.json',100,draw=['obs','exp0', 'exp1','exp2']))
  ...
  if first_loop:
    ...
    axis_g.SetPoint(axis_g.GetN(),1150,0.0)

    axis = CreateAxisHist(axis_g)
    axis.GetXaxis().SetTitle('m_{#phi} (GeV)')
    
    #axis.GetYaxis().SetTitle('95% CL upper limit on #sigma(pp#rightarrow Z* #rightarrow #phi A) #times BR(#phi#rightarrow#tau#tau) #times BR(A#rightarrow#tau#tau) (pb)')
    axis.GetYaxis().SetTitle('95% CL upper limit on #sigma #times BR(#phi#rightarrow#tau#tau) #times BR(A#rightarrow#tau#tau) (pb)')
    axis.GetYaxis().SetTitleOffset(1.75)
    axis.GetYaxis().SetTitleSize(0.037)
    axis.GetXaxis().SetTitleSize(0.04)
    axis.GetYaxis().SetLabelSize(0.03)
    axis.GetXaxis().SetLabelSize(0.03)

    axis.GetYaxis().SetRangeUser(10**(-18),1)
    ...
    # Standard CMS logo
    #DrawCMSLogo(pads[0], 'CMS', 'Supplementary', 11, 0, -0.05, 0.0, '', 0.8)
    DrawCMSLogo(pads[0], 'CMS','Supplementary', 1, 0.12, -0.045, 0.0, '', 0.8)

    #DrawCMSLogo(pads[0], 'CMS', "Supplementary", 1, 0.125, -0.07, 0, '', cmsTextSize=0.8)
  ...
  num = -1*ind
  latex.DrawLatex(graphs[ind]["exp0"].GetX()[graphs[ind]["exp0"].GetN()-1]+10,graphs[ind]["exp0"].GetY()[graphs[ind]["exp0"].GetN()-1],"m_{A} = %(mass)i GeV (#times 10^{%(num)i})" % vars())


  # Set the standard green and yellow colors and draw
  StyleLimitBand(graphs[ind],overwrite_style_dict=style_dict["style"])
  DrawLimitBand(pads[0], graphs[ind],draw=['exp2'], legend=legend if first_loop else None,legend_overwrite=style_dict["legend"] if first_loop else None)
  DrawLimitBand(pads[0], graphs[ind],draw=['exp1'], legend=legend if first_loop else None,legend_overwrite=style_dict["legend"] if first_loop else None)
  DrawLimitBand(pads[0], graphs[ind],draw=['obs','exp0'], legend=legend if first_loop else None,legend_overwrite=style_dict["legend"] if first_loop else None)
  legend.Draw()
...
canv.Print('mi_plots/model_independent_limit_all.pdf')
