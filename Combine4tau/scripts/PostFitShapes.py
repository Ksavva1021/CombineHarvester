...
for bin in bins_grouped:
  ...
  shapes_bkg_up = shapes_bkg[0].Clone()
  shapes_bkg_down = shapes_bkg[0].Clone()
...
#    # if you rebin the uncertainties for merged bins will be treated as being uncorrelated and summed in quadrature which will not be correct in general
    ...
    shapes_bkg_up = shape.Clone()
    shapes_bkg_down = shape.Clone()
  ...
  if args.postfit:
    ...
    nsamples_up = 0.
    nsamples_down = 0.
    nsamples_up_ = np.zeros(shapes_bkg[0].GetNbinsX())
    nsamples_down_ = np.zeros(shapes_bkg[0].GetNbinsX())
    ave_up = 0.
    ave_down = 0.
    for x in range(0, samples):
      ...
      if (shapes_bkg_var[0].Integral()-shapes_bkg[0].Integral()) > 0:
         ave_up += abs(shapes_bkg_var[0].Integral()-shapes_bkg[0].Integral())**2
         nsamples_up += 1
      else:
         ave_down += abs(shapes_bkg_var[0].Integral()-shapes_bkg[0].Integral())**2
         nsamples_down += 1
  
      for i in range(1, shapes_bkg[0].GetNbinsX()+1):
        err = shapes_bkg_var[0].GetBinContent(i)-shapes_bkg[0].GetBinContent(i)
        if err > 0:
           nsamples_up_[i-1]= nsamples_up_[i-1] + 1
           shapes_bkg_up.SetBinError(i, err*err + shapes_bkg_up.GetBinError(i))
        else:
           nsamples_down_[i-1]= nsamples_down_[i-1] + 1
           shapes_bkg_down.SetBinError(i, err*err + shapes_bkg_down.GetBinError(i))
    ...
    ave_up = (ave_up/float(nsamples_up))**.5
    ave_down = (ave_down/float(nsamples_down))**.5
    print 'Total Bkg = %.1f +/- %.1f (%.3f)' % (shapes_bkg[0].Integral(), ave, ave/shapes_bkg[0].Integral())
    print 'Total Bkg = %.1f + %.1f (%.3f) - %.1f (%.3f)' % (shapes_bkg[0].Integral(), ave_up, ave_up/shapes_bkg[0].Integral(),ave_down,ave_down/shapes_bkg[0].Integral())
    ...
    for i in range(1, shapes_bkg[0].GetNbinsX()+1):
      ...
      err_up = (shapes_bkg_up.GetBinError(i)/float(nsamples_up_[i-1]))**.5
      err_down = (shapes_bkg_down.GetBinError(i)/float(nsamples_down_[i-1]))**.5
      print("Error_tot",err_total)
      print("Error_down",err_down)
      print("Error_UP",err_up)
      print()
      shapes_bkg[0].SetBinError(i,err_total)
      shapes_bkg_up.SetBinError(i,err_up)
      shapes_bkg_down.SetBinError(i,err_down)
  ...
  shapes_bkg_up.SetName('TotalBkg_up')
  shapes_bkg_up.Write('TotalBkg_up')
  shapes_bkg_down.SetName('TotalBkg_down')
  shapes_bkg_down.Write('TotalBkg_down')
